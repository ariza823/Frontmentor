...
export default ({ ARRAY_IMGS, ARRAY_IMG_SMALL }) => {
  ...
  // Asegurarse de que ARRAY_IMGS y ARRAY_IMG_SMALL no están vacíos
  if (!ARRAY_IMGS || !ARRAY_IMG_SMALL || ARRAY_IMGS.length === 0 || ARRAY_IMG_SMALL.length === 0) {
    return <div>No hay imágenes para mostrar.</div>;  // Mensaje en caso de que no haya imágenes
  }

  return (
    <>
      {/* Sección con los productos en formato de cuadrícula */}
      <SlideProducts
